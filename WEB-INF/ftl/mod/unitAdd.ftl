    ...
    <section class="g-scrollview" id="app">
        ...
        <div class="m-cell">
			...
			<div class="cell-item">
                ...
                <div class="cell-right "><input type="text" name="ltleNo_s" class="cell-input" placeholder="" autocomplete="off" /></div>
            </div>
			<div class="cell-item">
				<div class="cell-left"><i class="cell-icon icon-phone1"></i>联系电话<sup>1</sup>：</div>
                <div class="cell-right "><input type="text" name="tleNo_s" class="cell-input" placeholder="" autocomplete="off" /></div>
            </div>
            <div class="cell-item">
				<div class="cell-left"><i class="cell-icon icon-phone1"></i>联系电话<sup>2</sup>：</div>
                <div class="cell-right "><input type="text" name="tleNoS_s" class="cell-input" placeholder="" autocomplete="off" /></div>
            </div>
        </div>
        <div class="m-cell">
            <div class="cell-item">
                <div class="cell-left">单位人员信息：</div>
                <textarea class="cell-textarea" placeholder="" name="unitNum_s"></textarea>
            </div>
        </div>
...
<script>
                ...
                methods: {
                    showData: function () {
                     ...
                     var s = queryUserGroup('<@security.authentication property="principal.username" />');
                        ...
                        var _self = this;
                        var baseUrl = 'http://'+changeAddr()+'/common/getModelList?length=200&filter[className]=industry&filter[type_s_s]=';
                        $.ajax({
                            type: 'GET',
                         ...
                         $.ajax({
                            type: 'GET',
                            url: baseUrl+'region&filter[id]='+(isNotNull(s)?s[2]:''),
                            success:function(data) {
    ...
    var dialog = YDUI.dialog;
    	var path = 'http://'+changeAddr()+'/common/saveModel?className=';
    		...
    		$('#msgForm').attr("action", path+'Unit&checkUniq=N&UID=<@security.authentication property="principal.username" />');
    		modVue.showData();
    });
