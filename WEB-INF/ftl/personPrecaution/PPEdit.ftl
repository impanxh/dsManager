...
<section class="g-flexview">
    ...
    <section class="g-scrollview">
        ...
        <form data-name="withdraw" name="submitForm" id="submitForm" class="ui-form"
              ...
              -->
        <h1 class="demo-pagetitle">更新人防</h1>
                        ...
                        <script id="imageTemplate" type="text/html">
                            <div id="{{mid}}">
                                    ...
                                    <div class="list-img">
                                        ...
                                        <button onclick='del("{{imageUrl}}","{{mid}}","img")' type="button">删除图片</button>
                                    </div>
...
<script>
    !function ($) {
        ...
        $("#submitBtn").on("click", function() {
            dialog.confirm('确认保存', '是否确定保存新的人防信息', [
                {
                    txt: '取消',
                    color: false,
                    callback: function () {
    ...
    function submit() {
        ...
        var settings = {
            "async": false,
            "crossDomain": true,
            "url": "http://" + changeAddr() + "/common/saveModel?className=PersonPrecaution&checkUniq=Y&UID=<@security.authentication property="principal.username" />",
            "method": "POST",
            "data": params
        }
        $.ajax(settings).done(function(response) {
            window.location.href = "${rc.contextPath}/personPrecaution/PPList?unit=" + $("#unitId").val();
...
//        }
        ...
        if( myPP != null) {
            ...
            if(myPP.s_Map.mediaFile_s != null) {
            	var _WORDJSON_MAP = {};
                ...
                $("#mediaFile").val(myPP.s_Map.mediaFile_s);
