		...
		<DIV class="summary mt10 uplan-summary-div">
			<UL class="uplan-detail-ul">
				...
				<LI style="width: 240px;"><SPAN>预留截止时间：</SPAN><SPAN class="con">
						<#if bodycontract.lockExpiredTime?? > <#if activityId ==
						"financialCheck" || activityId == "usertask2"> <a
						class="layer-tips" tip="点击设置" href="javascript:void(0);"
						onclick="setLockTime('${rc.contextPath}',${(bodycontract.id)!''})">
							<#setting
							date_format="yyyy-MM-dd">${(bodycontract.lockExpiredTime?date)!''}</a>
						<#else> <#setting
						date_format="yyyy-MM-dd">${(bodycontract.lockExpiredTime?date)!''}
						</#if> <#else> 未设置 </#if>
				</SPAN></LI> <#if bodycontract.remark?has_content>
