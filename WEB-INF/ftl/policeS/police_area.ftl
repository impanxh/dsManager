<#import "../template/template.ftl" as frame> <#global menu="辖区管理">
<@frame.html title="辖区管理"
...
<script type="text/javascript">
    ...
    function initTable () {
        table = $('#table').dataTable( {
            ...
            "ajax": {
                ...
                data: function(d) {
                 return $.extend( {}, d, {
                        "filter[type_s_s]" : "rsub",
                        "filter[pId_s_s]" : $('#psName').val(),
                        "filter[name_s_s]" : $('#xqName').val()
                    } );
                },
               "dataSrc": function (json) {
				   ...
				   var  list = queryByExampleV2({"filter[type_s_s]":"region"},"industry").content;
				   if(isNotNull(list)){
				    var mapJson  = {};
				   	$.each(list, function (i, item) { mapJson[item.id] = item.name_s});
  					$.each(json.content, function (i, item) { item.pName = mapJson[item.pId_s]});
  					}
                    return json.content;
                }
            },
    ...
    function initComplete() {
		...
		$("div#toolbar").html(
                '<div >' +
                        '    <span>派出所名称</span>' +
                        '        <select class="ui-input ui-input-mini" id="psName" style="width:135px"><option value="">请选择</option></select>' +
                        '</div>&nbsp;&nbsp;'+

                        '<div><span>辖区名称</span>&nbsp;'+
                        '<input class="ui-input" id="xqName" value=""></div>' +
                '</div>'

        );

        $('#psName,#xqName').change(function() {
            table.fnDraw();
        });

        $.ajax({
            url: 'http://'+changeAddr()+'/common/getModelListV2?filter[className]=industry&filter[type_s_s]=region',
            type : 'GET',
            async : false,
            dataType : 'json',
            success : function(data) {
                ...
                $.each(result,function(index, item) {
                    if(item != "") {
                        imageHtml+="<option value="+item.id+">" +item.name_s+ "</option>";
                    }
                });
                $('#psName').append(imageHtml);
            }
        });
    }
...
<form @submit.prevent="submit" method="post">
    <div  style="margin-top:30px;" class="ui-form-item"> <label class="ui-label mt10"><span class="ui-form-required">* </span>所属派出所名称： </label>
        <select class="ui-input ui-input-mini" v-model="pId" name="pId_s" id="pId_s">
            <option v-for="msg in policeList" :value="msg.id">{{msg.name_s}}</option>
        </select>
    </div>

    <div  style="margin-top:30px;" class="ui-form-item"> <label class="ui-label mt10"><span class="ui-form-required">* </span>辖区名称： </label>
...
<div class="withdraw-wrap color-white-bg fn-clear">
    <input type="hidden" id="type_s" name="type_s" value="rsub">
	<div class="withdraw-title">
		${menu!''} <a class="block-btn" id="addPS" href="javascript:void(0);">添加辖区</a>
	</div>
	<table id="table" class="display" cellspacing="0" width="100%">
		<thead>
			<tr>
                <th orderBy="name_s">辖区名称</th>
				<th>所属派出所</th>
				<th>备注</th>
				<th>操作</th>
			</tr>
