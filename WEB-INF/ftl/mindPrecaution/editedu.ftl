...
<body>
    <section class="g-flexview">
        <header class="m-navbar">
            ...
            <div class="navbar-center"><span class="navbar-title">教育记录信息</span></div>
        </header>
        ...
        <section class="g-scrollview">
            ...
            <div class="m-celltitle">教育记录内容</div>
            <div class="m-cell">
                <div class="cell-item">
        ...
        <div class="m-cell">
            ...
            <div class="cell-item">
            	<div class="cell-left"><i class="cell-icon icon-time"></i>发布时间：</div>
...
<script>
    !function ($) {
        ...
        $("#submitBtn").on("click", function() {
            dialog.confirm('确认保存', '是否确定保存教育记录信息？', [
            {
                txt: '取消',
                color: false,
                callback: function () {
    ...
    function uploadMediaFile() {
        ...
        var options = {
            ...
            success : function(data) {
                ...
                $("#imageShow").append(   template("imageTemplete", {imageUrl : data.path,})  );
            }
        };
    ...
    function submit() {
        ...
        if(editflag == 0)
        {
            var settings = {
                "async": false,
                "crossDomain": true,
                "url": "http://" + changeAddr() + "/common/saveModel?className=FireEdu&checkUniq=N&UID=<@security.authentication property="principal.username" />",
                "method": "POST",
                "data": params
            }
        }else{
            ...
            var settings = {
                "async": false,
                "crossDomain": true,
                "url": "http://" + changeAddr() + "/common/saveModel?className=FireEdu&checkUniq=Y&UID=<@security.authentication property="principal.username" />",
                "method": "POST",
                "data": params
            }
        }
        ...
        $.ajax(settings).done(function(response) {
            window.location.href = "${rc.contextPath}/mindPrecaution/listedu?unit="+$("#unitId_s").val();
    ...
    function getIssue(unitId) {
        $.ajax({
            "url" : "http://" + changeAddr() + "/common/queryById?className=FireEdu&id=" + unitId,
            type : 'GET',
            async : false,
            data : { },
            dataType : 'json',
