...
<section class="g-flexview">
    ...
    <section class="g-scrollview">
        ...
        <form data-name="withdraw" name="submitForm" id="submitForm" class="ui-form"
              ...
              -->
        <h1 class="demo-pagetitle">心防信息</h1>
...
<script>
    !function ($) {
        ...
        $("#recreateBtn").on("click", function() {
            dialog.confirm('心防信息更新', '是否确定要更新心防信息', [
                {
                    txt: '取消',
                    color: false,
                    callback: function () {
                ...
                },
                {
                    ...
                    callback: function () {
                        ...
                        window.location.href = "${rc.contextPath}/mindPrecaution/MPEdit?unit=" + $("#unitId").val() +
                            "&mpId=" + $("#mpId").val();
                    }
                }
            ]);
    ...
    function getLatestMindPrecaution(unitId, itemName) {
        $.ajax({
            url : "http://" + changeAddr() + "/common/getModelList",
            type : 'GET',
            async : false,
            data : {
                "filter[className]": "MindPrecaution",
                "length": 1024,
...
//        getLatestMindPrecaution(unitId, itemName);

        var mpId = getQueryString("mpId");
        var myMP = getPrecautionById("MindPrecaution", mpId)
        if( myMP != null) {
            $("#mpId").val(mpId);
            $("#itemName").text(myMP.s_Map.itemName_s);
            $("#updated").text($.format.date(myMP.updated_dt, "yyyy-MM-dd HH:mm:ss"));
            $("#detailDesc").val(myMP.s_Map.detailDesc_s);
            if(isNotNull(myMP.s_Map.mediaFile_s)){

                var _WORDJSON_MAP = {};
                if(isNotNull(myMP.s_Map.wordInfo_s)){
                    _WORDJSON_MAP =  JSON.parse(myMP.s_Map.wordInfo_s);
                }

                var arr = myMP.s_Map.mediaFile_s.split(',');
                var imageHtml = '';
                var wordHtml = '';
                $.each(arr, function(i,val){
