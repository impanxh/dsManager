...
<section class="g-flexview">
    ...
    <section class="g-scrollview">
        ...
        <form data-name="withdraw" name="submitForm" id="submitForm" class="ui-form"
              ...
              -->
        <h1 class="demo-pagetitle">技防信息</h1>
...
<script>
    !function ($) {
        ...
        $("#recreateBtn").on("click", function() {
            dialog.confirm('技防信息更新', '是否确定要更新技防信息', [
                {
                    txt: '取消',
                    color: false,
                    callback: function () {
                ...
                },
                {
                    ...
                    callback: function () {
                        ...
                        window.location.href = "${rc.contextPath}/techPrecaution/TPEdit?unit=" + $("#unitId").val() +
                            "&tpId=" + $("#tpId").val();
                    }
                }
            ]);
...
//        getLatestTechPrecaution(unitId, itemName);
        ...
        if( myTP != null) {
            ...
            $("#updated").text($.format.date(myTP.updated_dt, "yyyy-MM-dd HH:mm:ss"));
            $("#detailDesc").val(myTP.s_Map.detailDesc_s);
            if(isNotNull(myTP.s_Map.mediaFile_s)){
