...
<section class="g-flexview">
    ...
    <section class="g-scrollview">
        ...
        <form data-name="withdraw" name="submitForm" id="submitForm" class="ui-form"
              ...
              -->
        <h1 class="demo-pagetitle">更新技防</h1>
...
<script>
    !function ($) {
        ...
        $("#submitBtn").on("click", function() {
            dialog.confirm('确认保存', '是否确定保存新的技防信息', [
                {
                    txt: '取消',
                    color: false,
                    callback: function () {
    ...
    function submit() {
        ...
        var settings = {
            "async": false,
            "crossDomain": true,
            "url": "http://" + changeAddr() + "/common/saveModel?className=TechPrecaution&checkUniq=Y&UID=<@security.authentication property="principal.username" />",
            "method": "POST",
            "data": params
        }
        $.ajax(settings).done(function(response) {
            window.location.href = "${rc.contextPath}/techPrecaution/TPList?unit=" + $("#unitId").val();
...
//        }
        ...
        var myTP = getPrecautionById("TechPrecaution", tpId);
        if( myTP != null) {
            $("#tpId").val(tpId);
