	...
	function initUser(){
		userObj = queryForObject({"filter[gid_s_s]":$("#name").val()},'urole');
		if(isNotNull(userObj)){
			 $(":radio[name='r'][value='" + userObj.s_Map.rd_s + "']").prop("checked", "checked");
			 if( userObj.s_Map.rd_s  == 'hzadmin'){
					$("#roleOPer").hide ();
					$("#c2ALDiv").show();
			 }else  {
   ...
   function fetchData(selectAllTag) {
	...
	$.ajax({
		url : 'http://' + changeAddr() + '/roleTree?userid='+$("#name").val(),
		type : "GET",
		async : false,
		dataType : "json",
   ...
   function _fetchIndustry(){
	   ...
	   var arry = trimToDefault( ( isNotNull(userObj)&& isNotNull(userObj.s_Map)? userObj.s_Map.industry_s:""),"").split(",");
       $.ajax({
   			...
   			success:function(data){
   				var f='';
					  ...
					  {  index++;
					var hit =   $.inArray(data.id, arry);
					 	 var template1 ='<input type="checkbox" value="{0}" name="industry" id="region"'+(hit!=-1?"checked":"") +' />{1} &nbsp;&nbsp; ';
       ...
       var setting = {
		...
		function c2(){
			...
			var rd =$("input[name='r']:checked").val();;// $("#r").val();
			var sString =[];
			var checkPlaceCount =0;//民警检查
			var checkPlaceAdminCount= 0 ;//派出所领导检查
			var _industry = [];
			var _region  = [];
			var _place=[];//派出所
			...
			var uid = $("#name").val();
			  var settings = {
				        ...
				        "method" : "GET",
				        "data" : {'dByField':'gid_s_s','value':uid}
				    }
				    $.ajax(settings).done(function(response) {
			    		...
			    		var postData={"placeIds_s":_place,"rd_s":rd,"roleTree_s": sString,"region_s":_region,"industry_s":_industry,"gid_s":uid,"checkUniq":'N'};
