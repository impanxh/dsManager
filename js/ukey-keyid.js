...
function fillUKeyInfo4MSIE() {
    ...
    try {
		...
		if(username != 'inTheFirstPlace') {
			alert("UKey已经授权使用，请先解除绑定，再使用此UKey");
			return false;
		}
...
function writeUKey4MSIE(username, password, keyid)
{
    try
    {
...
function writeUKey(username, password, keyid)
{
    if(username == "") {
        alert("用户名不能为空。");
        return false;
    ...
    if(isMSIE()) {
        return writeUKey4MSIE(username, password, keyid);
    }
